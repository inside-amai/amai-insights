...
const Paragraph = ({ children }: { children: string }) => {
  ...
  if (startsWithStage && firstSentence) {
    return (
      <div className="grid gap-3 border-t border-white/10 py-6 first:border-t-0 md:grid-cols-[8rem_1fr] md:gap-8">
        <p className="text-base font-medium text-[#B4F6AD]">{firstSentence[1]}</p>
        <p className="text-base font-light leading-8 text-white/62 md:text-lg">{firstSentence[2]}</p>
      </div>
    );
  }

  return <p className="text-base font-light leading-8 text-white/62 md:text-lg md:leading-9">{children}</p>;
};
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <section id="summary" className="bg-perspective-grid px-5 py-20 md:px-8 md:py-32">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-12 lg:gap-20">
          <header className="lg:col-span-4">
            ...
            <h2 className="mt-5 text-4xl font-light md:text-6xl">The record is the product.</h2>
          </header>
      ...
      <div className="border-y border-white/10 bg-black px-5 py-5 lg:hidden">
        <label htmlFor="chapter-nav" className="mb-2 block font-mono text-[9px] uppercase text-white/35">Jump to chapter</label>
        <select
          id="chapter-nav"
      ...
      <section className="bg-perspective-grid px-5 py-28 text-center md:px-8 md:py-40">
        <div className="mx-auto max-w-4xl">
          <p className="font-mono text-[10px] uppercase text-[#B4F6AD]/60">Every move, on the record</p>
          <h2 className="mt-6 text-4xl font-light leading-tight md:text-7xl">Check the operator before you trust it.</h2>
          <div className="mt-12 flex flex-col items-center justify-center gap-6 text-sm font-light sm:flex-row sm:gap-10">
            <a href="https://bureau.amai.net" target="_blank" rel="noopener noreferrer" className="border-b border-[#B4F6AD]/35 pb-1 text-[#B4F6AD]/80 transition-colors hover:text-[#B4F6AD] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#B4F6AD]">See the Bureau ↗</a>
            <a href="/methodology" className="border-b border-white/20 pb-1 text-white/60 transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#B4F6AD]">How TARI™ is built →</a>
            <a href="/launchpad" className="border-b border-white/20 pb-1 text-white/60 transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#B4F6AD]">Launch with an operator →</a>
          </div>
        </div>
      </section>
